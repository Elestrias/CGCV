	...
	struct unsigned_color
	{
		static unsigned_color from_color(const color& color)
		{
			return unsigned_color{
					static_cast<uint8_t>(std::clamp(color.r * float(225), float(0), float(225))),
					static_cast<uint8_t>(std::clamp(color.g * float(225), float(0), float(225))),
					static_cast<uint8_t>(std::clamp(color.b * float(225), float(0), float(225)))
			};
		};
		static unsigned_color from_float3(const float3& color)
		{
			float3 clmpd = clamp(float(225) * color, float(0), float(225));
			return unsigned_color{
					static_cast<uint8_t>(clmpd.x),
					static_cast<uint8_t>(clmpd.y),
					static_cast<uint8_t>(clmpd.z)
			};
		};
		float3 to_float3() const
		{
