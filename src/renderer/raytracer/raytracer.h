...
namespace cg::renderer
{
	...
	{
		a = float3{vertex_a.x,
				   vertex_a.y, vertex_a.z};
		b = float3{vertex_b.x,
				   vertex_b.y, vertex_b.z};
		c = float3{vertex_c.x,
				   vertex_c.y, vertex_c.z};

		ba = b - a;
		ca = c - a;

		na = float3{vertex_a.nx,
					vertex_a.ny, vertex_a.nz};
		nb = float3{vertex_b.nx,
					vertex_b.ny, vertex_b.nz};
		nc = float3{vertex_c.nx,
					vertex_c.ny, vertex_c.nz};

		ambient = float3{vertex_a.ambient_r,
						 vertex_a.ambient_g, vertex_a.ambient_b};
		diffuse = float3{vertex_a.diffuse_r,
						 vertex_a.diffuse_g, vertex_a.diffuse_b};
		emissive = float3{vertex_a.emissive_r,
						  vertex_a.emissive_g, vertex_a.emissive_b};
	}
		...
		for (size_t i =  0; i < index_buffers.size(); ++i) {
			...
			while (index <index_buffer->get_number_of_elements()) {
				triangle<VB> triangle(
						vertex_buffer->item(index_buffer->
											item(index++)),
						vertex_buffer->item(index_buffer->
											item(index++)),
						vertex_buffer->item(index_buffer->
...
#pragma omp parallel for
			...
			for (int x = 0; x < width; ++x) {
				for (int y = 0; y < height; ++y) {
					float u = (2.f * x + jitter.x)
									  / static_cast<float>(width) - 1.f;
					float v = (2.f * y + jitter.y)
					...
					history->item(x, y) +=
							sqrt(payload.color.to_float3() / accumulation_num);
			...
			const ray& ray, size_t depth, float max_t, float min_t) const
	{
		...
		for (auto& aabb: acceleration_structures) {
			...
			for ( auto& triangle: aabb.get_triangles()) {
				...
				if ( payload.t > min_t &&
							payload.t < closest_hit_payload.t) {
