...
namespace cg::renderer
{
	...
	{
		for (unsigned int i = 0; i < render_target->get_number_of_elements(); i++) {
			render_target->item(i) = in_clear_value;
		}


		for (unsigned int i = 0; i < depth_buffer->get_number_of_elements(); ++i) {
	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes) {
			...
			const float edge = edge_function(vertex_a, vertex_b, vertex_c);
			for (float x = bounding_box_begin.x; x <= bounding_box_end.x; x += 1.f) {
				for (float y = bounding_box_begin.y; y <= bounding_box_end.y; y += 1.f) {
					float2 point{x, y};

					const float edge0 = edge_function(vertex_a, vertex_b, point);
					const float edge1 = edge_function(vertex_b, vertex_c, point);
					const float edge2 = edge_function(vertex_c, vertex_a, point);

					if (edge0 >= 0.f && edge1 >= 0.f && edge2 >= 0.f) {
						const auto u_x = static_cast<size_t>(x);
						const auto u_y = static_cast<size_t>(y);
						const float u = edge1 / edge;
						const float v = edge2 / edge;
						const float w = edge0 / edge;
