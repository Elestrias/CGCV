...
{
	DirectX::XMFLOAT3 eye_position{position.x,
								   position.y,
								   position.z};

	DirectX::XMFLOAT3 eye_direction{get_direction().x,
									get_direction().y,
									get_direction().z};

	DirectX::XMFLOAT3 up_direction{get_up().x,
								   get_up().y,
								   get_up().z};

	return DirectX::XMMatrixLookAtRH(DirectX::XMLoadFloat3(&eye_position),
...
{
	return DirectX::XMMatrixPerspectiveFovRH(angle_of_view,
											 aspect_ratio,
											 z_near,
											 z_far
	);
